...
## 8. 第九轮：现货卖出质量（Codex）— 已跑结果

- **命令**：`python ab_test_v3.py --live --spot`（或 `RUN_LIVE_AB=1 RUN_LIVE_SPOT=1`）。
- **基线**：run#46（实盘口径 LVT+35）。
- **变体**：B 确认SS70min2、C 确认SS60min2、D sell_thr22、E spot_cd18、F spot_cd24、**G SS70min2+cd18**。
- **结果摘要**（2025-01～2026-01）：

| 变体 | 收益 | MaxDD | pPF | CLOSE_SHORT | SPOT_SELL |
|------|------|-------|-----|-------------|-----------|
| A:run46基线 | +738.40% | -6.83% | 1.84 | -44,896 | +219,685 |
| **G:SS70min2+cd18** | **+760.70%** | **-6.56%** | **1.96** | **-42,614** | **+263,852** |
| E:spot_cd18 | +734.71% | -6.57% | 1.86 | -42,614 | +211,764 |
| C:确认SS60min2 | +740.13% | -6.83% | 1.85 | -44,889 | +223,144 |

- **结论**：**G（确认 SS70 min2 + spot_cooldown 18）** 最优：收益 +22pp、pPF 1.96、回撤略优、CLOSE_SHORT 与 SPOT_SELL 双改善。可选晋级为实盘口径新默认。
